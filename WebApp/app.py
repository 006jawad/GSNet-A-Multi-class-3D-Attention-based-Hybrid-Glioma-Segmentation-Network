from my_model import GSNet, ShowResult
import os
import warnings
import numpy as np
...
weight_path='weight/attu_net.pth'

MODEL = GSNet(img_ch=4, output_ch=3).to(device=device)
...
def upload_predict():
    if request.method == "POST":
        ...
        process(imgs_loc,image_name[:12])
        ...
        return render_template("index.html", image_loc = (f"{image_name[:12]}_out.png"),prediction=image_name[:12])
